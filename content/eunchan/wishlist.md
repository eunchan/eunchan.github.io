+++
title = "Wish List"
slug = "wishlist"
date = 2009-01-01
updated = 2026-09-14
+++

# Wish List

제가 가지고 싶은 것은 끝없이 많지만, 그 중에 꽤 비싸거나, 크게 의미 있는 것들을
정리해 보았습니다. 그때 그때 달라지긴 해서, 있던 목록이 지워지기도 합니다.

### Leica Q Series

Leica에서 나온 렌즈 일체형 풀프레임 카메라입니다. 기능상으로 다른 풀프레임과
그다지 차이가 없지만, 라이카라는 이름만으로 한번 쯤 소유해보고 싶은
카메라입니다.

### Class B 캠핑카

캐러밴을 끌다보니 이게 상당히 불편하더라구요. 지금 캐러밴은 아니지만 모터사이클
트레일러를 끌고 고속도로를 가다가 볼 너트가 풀려서 깜짝 놀란 경험이 있어서인지,
뭔가를 끌고 가기만 하면 뒤에 잘 따라오는지 신경쓰느라 상당히 스트레스를
받습니다. 그래서 뒤에 끌고 다닐 필요가 없는 Class B 캠핑카, 그중에서 가벼운
오프로드를 갈 수 있는 Storyteller나 Winnebago Revel 같은 캠핑카가 있으면
좋겠다는 생각을 합니다.

----

## Done

### KTM 790 Adventure / Adventure R

2020년에 구입하였습니다. [구입기](../motorcycle/ktm-790-adventure-r.md)

### [Orion XT-8](../sky/equip/xt8.md)

### Sky Quality Meter - L

관측시 하늘의 상태가 어떻게 되는지는 항상 육안과 추측에 의존하는데 이 기계는
하늘의 상태를 객관적인 수치로 표현합니다. 정확한 통계치를 내는데에는 아주 좋을
듯 하네요.

[Unihedron SQM-L](http://unihedron.com/projects/sqm-l/)

### Solo Stove Titan / Bonfire

Bonfire는 가족과 캠핑갈 때에나 뒷마당에서 쓰기에 좋아보이고, Titan은 혼자서 여행할 때 딱 좋아보이는 물건이네요.

### Triumph Bonneville T100

![Triumph Bonneville T100](/media/page/eunchan/bonneville-t100.jpg)

Triumph사 의 클래식 바이크로 50여년을 넘게 지속되어온 모델입니다.

**March 8, 2014** T100의 스포크휠을 뒤로 하고 일반 모델 Bonneville을 구입하였습니다.
[Bonneville 구입](../motorcycle/bonneville.md) 글 참조.
